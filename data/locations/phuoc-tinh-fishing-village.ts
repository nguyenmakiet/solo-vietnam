import { Location } from "@/data/location"
import { heroUrl } from "@/lib/cloudinary"

export const phuocTinhFishingVillage: Location = {
  slug: "phuoc-tinh-fishing-village",
  name: "Phước Tỉnh Fishing Village",
  updatedAt: "2026-09-21",
  provinces: ["vung-tau"],
  destination: "",
  lat: 10.402412684439296,
  lng: 107.17863802885124,
  address: "Phước Tỉnh, Long Điền, Bà Rịa - Vũng Tàu",
  type: ["nature", "cultural", "beach"],
  categories: ["food", "culture"],
  experiences: ["food", "culture", "walking-tour", "markets"],
  tags: ["🎣 Active Fishing Port", "🦐 Fresh Seafood", "🚢 Fishing Fleet"],
  bestTime: "4:00-7:00 AM for the fish market at full activity; 7-9 AM is pleasant for a slower walk watching daily life; 4-6 PM brings a calmer mood, good for walking the harbourside embankment and watching sunset over the port",
  bestMonths: [1, 2, 3, 4, 11, 12],
  entranceFee: "Free",
  openingHours: "Fish market most active 4:00 – 8:00 AM daily",
  mapUrl: "https://www.google.com/maps?q=10.402412684439296,107.17863802885124",
  streetView: { embedUrl: "https://www.google.com/maps/embed?pb=!4v1775294205918!6m8!1m7!1s8lhHJy4t5YmRwzCB4tLj4A!2m2!1d10.4023582122651!2d107.1786123554698!3f358.69104477515555!4f-1.2676060829538898!5f0.7820865974627469", },
  heroImage: heroUrl("phuoc-tinh-fishing-village-vung-tau-7_teb7az"),
  gallery: [
    "phuoc-tinh-fishing-village-vung-tau_bqqiuk",
    "phuoc-tinh-fishing-village-vung-tau-6_aivcsj",
    "phuoc-tinh-fishing-village-vung-tau-5_oceqle",
    "phuoc-tinh-fishing-village-vung-tau-4_rwtuew",
    "phuoc-tinh-fishing-village-vung-tau-3_swteec",
    "phuoc-tinh-fishing-village-vung-tau-2_g0ksya",
    "phuoc-tinh-fishing-village-vung-tau-9_zdnkhz",
    ],
  seoDescription:
    "Phước Tỉnh is one of the oldest and most active fishing ports in southern Vietnam - a working harbor in Long Điền district, Bà Rịa-Vũng Tàu, where hundreds of fishing vessels dock daily, and the morning market is one of the most photogenic and genuine seafood scenes near Ho Chi Minh City. Note: don't confuse it with Phước Hải, a similarly-named but separate fishing town in neighbouring Đất Đỏ district.",
  tips: [
    "Time needed: 2-4 hours if you want to slow down, watch the market, and enjoy some food rather than just pass through",
    "Don't confuse this with Phước Hải (Bờ Kè Phước Hải) - a different, similarly-named fishing town in Đất Đỏ district, not Long Điền. They're both worth visiting but are separate places",
    "Arrive between 4:00 and 7:00 AM to see the boats return and the market at full activity - by 9:00 AM the best seafood is sold and the harbor quiets significantly",
    "The harbor viewing point above the fish market gives the best overview of the fleet - hundreds of blue and red fishing boats packed into the harbor is one of the more striking visual scenes in southern Vietnam",
    "Fresh seafood bought directly at the market is extremely cheap compared to Vũng Tàu city restaurants - bring a cooler bag if driving and buy dried squid, dried shrimp, and fresh catch to take back",
    "The surrounding Long Điền and Đất Đỏ district beaches (Phước Hải, Lộc An) are quieter alternatives to Vũng Tàu's main beaches and easily combined in the same day trip",
    "The road from Vũng Tàu to Phước Tỉnh along the coast passes through some of the least-developed coastline in southern Bà Rịa - the drive itself is worth doing slowly",
    "There are plenty of small snack food stalls along the waterfront if you want to graze rather than eat a full meal",
    "Phước Tỉnh leans simple and unhurried rather than crowded or lively like Vũng Tàu proper - it has its own harbourside embankment for a stroll, and Dinh Cô temple in nearby Long Hải is an easy add-on if you have more time",
  ],
  content: {
    intro:
      "Phước Tỉnh is a large fishing commune of over 300 years' standing in Long Điền district, Bà Rịa-Vũng Tàu province, and one of the oldest and most productive fishing ports in southern Vietnam - sometimes called Vietnam's richest fishing village. The village formed early in the region's settlement, sitting east of the Cửa Lấp river mouth on a strip of land that juts into the sea in a distinctive plowshare shape, with clear water and a gently sloping beach. According to the historical gazetteer Đại Nam Nhất Thống Chí, locals built Đàn Kỳ Phong here as early as the Gia Long period (1802-1820) to worship sea deities - a reflection of how long fishing has anchored life in this town. Per capita income here has been cited around 2,500 USD a year, well above many surrounding areas, and the village has earned a 'billionaire village' reputation locally, with a number of fishermen owning paired trawlers worth tens of billions of đồng. Beyond fishing itself, Phước Tỉnh has built real wealth from shipbuilding: five shipyards here employ hundreds of skilled tradespeople and many more local labourers, launching upwards of 40 offshore fishing vessels a year, and the town has a strong reputation for ship repair, drawing boats from as far as Quảng Ngãi, Phú Yên, and Kiên Giang. Today the harbor holds hundreds of fishing vessels - trawlers, gill-netters, and the round basket boats (thuyền thúng) used for nearshore work - and the morning fish market that operates on the beach and dock from before dawn remains a genuine scene of commercial fishing activity, though in recent years the offshore fishing industry has slowed somewhat as marine resources have declined, giving the village a bit more of its old, unhurried character back. The town has grown up entirely around the fishing and shipbuilding industries: fish sauce factories, dried seafood processing, ice plants, boat repair yards, and the dense residential fabric of a community that has lived from the sea for generations. It is not a built-up tourist destination, but it is one of the most visually compelling and authentic working port experiences accessible from Ho Chi Minh City, with its own harbourside embankment for a quieter evening walk. Note: this is a different place from Phước Hải, a similarly-named fishing town in neighbouring Đất Đỏ district that has its own well-known waterfront promenade (bờ kè) and seafood dining scene - easy to confuse by name, but a separate destination.",
    howToGetThere:
      "Phước Tỉnh is approximately 100km from Ho Chi Minh City, in Long Điền district of Bà Rịa-Vũng Tàu. From HCMC, the most direct route is via the Long Thành-Dầu Giây Expressway to Bà Rịa city, then Provincial Road 44A south toward Long Điền and Phước Tỉnh - total driving time about 1.5-2 hours. From Vũng Tàu city, Phước Tỉnh is roughly 15-20km away and takes around 25-35 minutes by road, depending on traffic and route - a scenic coastal drive worth taking slowly. By bus from HCMC, services run to Bà Rịa and Long Điền; motorbike taxi covers the final stretch. For the early morning market, driving or riding from HCMC the night before and staying in Long Hải or Long Điền is the most practical approach.",
    whatToExpect:
      "The harbor area is centered around the fish market dock and the boat anchorage. From before dawn, fishing vessels return and unload directly onto the dock where buyers - wholesalers, restaurant owners, and individual shoppers - bid for and purchase the catch. The range of seafood is exceptional: shrimp, squid, crab, various fish species, and shellfish, all live or freshly caught. The market is loud, wet, and smells strongly of the sea - an environment of genuine commercial activity rather than tourist performance. The harbor itself, viewed from the elevated road above, shows the full scale of the fishing fleet anchored in rows - nearby ports in the same district (Tân Phước, Phước Hiệp) are so busy during fishing season that hundreds of boats can be waiting for space meant for a fraction of that number. The surrounding streets have dried seafood shops, fish sauce producers, and basic seafood restaurants serving the catch at low prices.",
    travelTips:
      "Phước Tỉnh is the answer to the question of what Mũi Né's fishing village used to feel like before resort development enclosed it. The scale is larger - this is a proper fishing port, not a village - and the activity is more commercial, but the authenticity is complete. If you're based in HCMC and want a day trip that combines genuine working Vietnam with good food and a beach option afterward, the Phước Tỉnh morning market followed by Phước Hải or Lộc An beach in the afternoon is one of the best day-trip circuits available from the city. The drive along the Bà Rịa coast is underrated as a motorbike road.",
  },
  insights: {
    highlights: [
      "One of southern Vietnam's oldest fishing ports, with a documented history stretching back to the Gia Long era (1802-1820) and a local reputation as Vietnam's 'billionaire fishing village'",
      "A pre-dawn fish market where hundreds of boats unload directly onto the dock for wholesale buyers and shoppers",
      "A genuinely working town built entirely around the fishing industry, with none of the visitor infrastructure of nearby beach destinations",
    ],
    thingsToKnow: {
      crowds: "Busy with fishermen, wholesalers, and locals in the early morning; very few tourists at any time",
      difficulty: null,
      safety: "This is a working harbor, not a swimming beach - watch your footing on wet docks, and be mindful of boat and vehicle traffic during the busy early-morning unload",
      accessibility: "No formal visitor facilities; the harbor viewing point above the market is freely accessible",
      seasonal: "Dry season (roughly November-April) brings calmer seas and generally higher fishing activity",
    },
    visitorTips: [
      "Arrive between 4 and 7 AM for the market at its most active - later than 9 AM and most of the best catch is already sold",
      "Bring a cooler bag if you're driving and want to take fresh or dried seafood home - prices here are well below city restaurants",
      "Combine with Phước Hải or Lộc An beach in the afternoon for a fuller day trip, but don't confuse Phước Tỉnh's fish market with Phước Hải's separate waterfront promenade",
    ],
    faq: [
      { question: "Is Phước Tỉnh the same as Phước Hải?", answer: "No - they're separate fishing towns with similar names in different districts (Phước Tỉnh in Long Điền, Phước Hải in Đất Đỏ). Phước Tỉnh is centred on its working fish market and harbor; Phước Hải has its own waterfront promenade (bờ kè) with seafood restaurants and sunset views. Both are worth visiting, but plan for them separately." },
      { question: "What's the best time to visit the fish market?", answer: "Between 4:00 and 7:00 AM, when boats are returning and unloading. By 9:00 AM, the best seafood has usually been sold and the harbor quiets down significantly." },
      { question: "Is there anything to do here besides the fish market?", answer: "The main draw is the market and the harbor itself - watching the fleet, buying fresh or dried seafood, and taking in a genuine working port. There's no separate tourist attraction beyond that, which is part of its appeal." },
    ],
    sentiment: {
      positive: "Visitors praise the authenticity of the working harbor, the sheer scale of the fishing fleet, and the availability of very fresh, cheap seafood direct from the dock.",
      negative: "Some visitors expecting a polished tourist destination are surprised by the complete lack of visitor infrastructure - this is a working port first, and it doesn't cater to sightseers.",
    },
  },
}
