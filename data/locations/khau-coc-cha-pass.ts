import { Location } from "@/data/location"
import { heroUrl } from "@/lib/cloudinary"

export const khauCocChaPass: Location = {
  slug: "khau-coc-cha-pass",
  name: "Khau Cóc Chà Pass",
  provinces: ["cao-bang"],
  destination: "cao-bang",
  lat: 22.926557070752953,
  lng: 105.78388981658192,
  address: "Xuân Trường, Bảo Lạc, Cao Bằng",
  type: ["nature", "mountain"],
  categories: [],
  experiences: ["motorcycling", "photography", "trekking"],
  tags: ["🏍️ 15-Tier Pass", "📸 Panoramic Views", "🌀 Hairpin Switchbacks", "🏔️ Highland Road"],
  bestTime: "For the safest road conditions, Nov - Apr (the dry season) is the more reliable window - drier pavement and lower landslide risk on a road where that genuinely matters. Aug - Oct offers a real scenic bonus (golden rice ripening in the valley below) and Feb - Mar brings wildflowers, but both fall partly or fully within the wetter months, so weigh the extra scenery against riskier riding conditions. Avoid riding in heavy rain regardless of season",
  bestMonths: [11, 12, 1, 2, 3, 4],
  entranceFee: "Free; the viewpoint hike has an informal 10,000 VND motorbike parking fee at the small shop where the trail starts",
  openingHours: "Open 24/7",
  mapUrl: "https://www.google.com/maps?q=22.926557070752953,105.78388981658192",
  streetView: {embedUrl: "https://www.google.com/maps/embed?pb=!4v1775823018834!6m8!1m7!1sCAoSFkNJSE0wb2dLRUlDQWdJRC0ydjc2VFE.!2m2!1d22.92647298469297!2d105.7791502882675!3f348.8890528575274!4f-35.60972270022248!5f0.7820865974627469", },
  heroImage: heroUrl("khau-coc-cha-pass-hero_placeholder"),
  gallery: [],
  seoDescription:
    "Khau Cóc Chà Pass, sometimes called the '15-tier pass,' is one of Cao Bằng's most dramatic mountain roads - just 2.5km long but packed with 14 hairpin switchbacks connecting Xuân Trường commune to Bảo Lạc town near the Chinese border, with a short hike leading to the best panoramic viewpoint over the whole switchback sequence.",
  tips: [
    "Despite being only about 2.5km long, the pass has 14 hairpin switchbacks stacked into roughly 15 visible tiers on the hillside - it can take over an hour to ride carefully, and isn't a good fit for inexperienced riders or automatic scooters",
    "The classic photo isn't from the road itself - it's from a viewpoint reached by a roughly 2km hike along the mountainside, starting near a small shop at the base of the pass. Budget 20-30 minutes each way, wear real hiking shoes (the trail has steep, sometimes muddy sections), and expect an informal 10,000 VND fee for parking your motorbike at the shop",
    "Don't confuse Khau Cóc Chà with Mẻ Pia Pass nearby - they're two different passes on the same general route toward Bảo Lạc, and locals will clarify which is which if you ask",
    "Avoid riding this pass in heavy rain - sections have had landslide and rockfall issues, and the tight switchbacks with loose gravel at the edges get considerably more dangerous when wet",
    "The nearby village at the top of the pass, which the pass takes its name from, is a good spot to try local specialties like thắng cố, corn wine, or sticky rice with tram fruit if you have time to stop",
    "Fuel up before setting out - this is a remote stretch of highway with no petrol stations directly on the pass itself",
  ],
  content: {
    intro:
      "Khau Cóc Chà Pass, often called the '15-tier pass' (đèo 15 tầng), is a short but dramatic stretch of National Highway 4A in Bảo Lạc district, Cao Bằng, connecting Xuân Trường commune to Bảo Lạc town near the Chinese border. In the Tày language, 'khau' means mountain pass, while 'Cốc Chà' refers to both a small Tày village near the summit and a tree species common in the area - not, as sometimes claimed, a description of the terrain itself. Despite covering barely 2.5km, the road packs in 14 tight hairpin switchbacks, stacking into roughly 15 visible tiers when viewed from a distance - a sight often compared to a ribbon or a staircase draped across the mountainside. The pass began as a narrow, roughly 40cm-wide footpath dating to the French colonial period, used mainly for foot and horse traffic, before the province invested in a full upgrade between 2009 and 2011 - widening the road to around 5 metres, paving it, and easing the sharpest bends for safety. It isn't included among Vietnam's famous 'Four Great Passes' (Pha Đin, Ô Quy Hồ, Mã Pí Lèng, and Khau Phạ), but it has become an increasingly popular stop for younger domestic travellers and adventure riders exploring Cao Bằng.",
    howToGetThere:
      "There are two main approaches. From Cao Bằng city, the more direct route follows Highway 3 to Bảo Lạc town (roughly 90-100km, around 3 hours by motorbike), then onto QL4A to the pass. From Hà Giang, a longer but more scenic option runs via QL4C to Yên Minh, then QL4A over Mã Pí Lèng Pass before reaching Khau Cóc Chà - a good way to combine two of the north's most dramatic mountain roads in one trip, if you have the time. A motorbike is the practical choice for either route, given the flexibility it offers on these roads; if travelling by car, a high-clearance vehicle with a reasonably powerful engine is worth insisting on. The road is paved but narrow and steep, with ongoing safety improvement work reported on some of the more accident-prone sections, so delays or detours are possible and may not show up on Google Maps - allow extra time and ride cautiously.",
    whatToExpect:
      "The road climbs and descends through the 14 switchbacks in quick succession, with a sheer rock face on one side and a steep drop on the other for much of the route - genuinely intense riding that rewards a steady hand rather than speed. There's a small flat clearing right at the top of the pass itself, a good spot to catch your breath, but it's not the best view. For the fuller, classic panorama of all the switchbacks stacked together, hike up to Đỉnh Pác Thốc (Pác Thốc Peak), known locally as the 'roof of Bảo Lạc,' on the mountain facing the pass - about 30-40 minutes each way, with sections of sharp, jagged limestone ('đá tai mèo') and stone steps that demand reasonable fitness. Budget around 1.5 hours round trip if you want time for photos at the top, and note there are no shops or vendors anywhere along this trail, so bring your own water and snacks. The trail is manageable in the dry season but gets genuinely slippery on the rock steps when wet. Cloud and mist are common, especially in the morning, adding to the atmosphere but sometimes obscuring the view entirely - if that happens, waiting isn't a bad strategy, since conditions can shift within 20-30 minutes.",
    travelTips:
      "Khau Cóc Chà is a landscape and riding experience rather than a single sight to check off - the switchbacks themselves and the hike to the viewpoint are the point, not a specific building or landmark. It suits travellers in good health who genuinely enjoy dramatic mountain roads and don't mind a moderately demanding trek for the payoff - it's a poor fit for anyone without decent fitness, or with cardiovascular conditions, given the combination of tense, blind-corner riding and a real uphill hike to the best viewpoint. It fits naturally into a Cao Bằng loop that also takes in Bản Giốc Waterfall, Ngườm Ngao Cave, and the Quây Sơn River, though the Bảo Lạc area is far enough from those better-known spots that it works best as part of a dedicated northwestern Cao Bằng route rather than a quick add-on. Riders who've done the Ha Giang Loop and want a shorter, similarly intense switchback experience without the crowds will find this a worthwhile detour - and combining it with Mã Pí Lèng Pass via the Hà Giang approach route makes for one of the more memorable stretches of riding in northern Vietnam.",
  },
  insights: {
    highlights: [
      "14 hairpin switchbacks stacked into roughly 15 tiers on a near-vertical mountainside, packed into just 2.5km of road",
      "A roughly 30-40 minute hike up to Đỉnh Pác Thốc, the mountain facing the pass, leads to the best panoramic viewpoint over the full switchback sequence",
      "Part of a scenic loop route through Cao Bằng's border districts near China, with dramatic mountain scenery throughout the drive",
    ],
    thingsToKnow: {
      crowds: null,
      difficulty: "Steep, tightly winding mountain road with loose gravel on the edges in places - well suited to travellers in good health who enjoy dramatic mountain roads, but not recommended for anyone without decent fitness or with cardiovascular conditions, given the combination of tense riding and a genuinely uphill hike to the best viewpoint. The Pác Thốc viewpoint trail has rocky, uneven sections and stone steps. A manual or semi-automatic motorbike is recommended over a fully automatic scooter for the ride itself.",
      safety: "Road conditions vary - some sections have had mud, loose rocks, potholes, or ongoing repair work. Avoid riding in rain, since landslide risk increases significantly. GPS can lag or mislead near the Chinese border area - stick to the main highway (QL4A) rather than trusting an alternate route suggested by an app. Several hikers have reported falls on the descent from the viewpoint trail; proper hiking shoes are genuinely important here.",
      accessibility: "The viewpoint itself is free to access. Motorbike parking at the trailhead shop costs around 10,000 VND. The hiking trail has steep sections with high, uneven steps that may be difficult for those with mobility limitations or shorter strides.",
      seasonal: "Rainy conditions make the road and trail considerably more hazardous and raise landslide risk - best avoided. Ongoing road repairs in places may cause delays not reflected on mapping apps.",
    },
    visitorTips: [
      "Park at the small shop near the base of the pass (around 10,000 VND), then follow the marked trail (often indicated with red-painted arrows) to reach the viewpoint hike",
      "Wear sturdy hiking shoes with real grip - the final stretch of the viewpoint trail is steep, and muddy sections get slippery fast",
      "Bring water, or buy some from the vendors near the trailhead - the hike takes 20-35 minutes each way depending on fitness and pace",
    ],
    faq: [
      { question: "How far is the pass from Cao Bằng city or Bảo Lạc town?", answer: "The pass is around 90-100km from Cao Bằng city (roughly 3 hours by motorbike given the mountain roads) and about 16-20km from Bảo Lạc town itself along QL4A." },
      { question: "What is the road condition like, and is it safe for motorbikes?", answer: "The road is mostly paved but narrow, steep, and tightly winding, with loose gravel at the edges in places and occasional repair work. Riding in the rain significantly raises landslide risk and isn't recommended. A manual or semi-automatic motorbike, and some riding experience, are strongly recommended over a basic automatic scooter." },
      { question: "Is there a fee to visit the viewpoint, and how do I get there?", answer: "The viewpoint (Đỉnh Pác Thốc, the peak facing the pass) is free to visit. From the base of the pass, follow the marked trail on foot - roughly 30-40 minutes each way, with rocky and stepped sections - to reach the panoramic overlook of the switchbacks. Budget about 1.5 hours round trip if you want time for photos, and bring your own water and snacks since there's nothing available along the trail." },
    ],
    sentiment: {
      positive: "Visitors consistently praise the dramatic switchbacks, the striking panoramic views from the hiking viewpoint, and the sense of having found a genuinely off-the-beaten-path stretch of northern Vietnam.",
      negative: "Some visitors note road quality concerns - crumbling sections, potholes, and occasional rockfall - along with ongoing construction causing delays, a steep and occasionally dangerous descent on the viewpoint hike for less experienced walkers, and loose gravel that makes the ride genuinely risky for less confident motorcyclists, especially in wet conditions.",
    },
  },
}
