import { Location } from "@/data/location"
import { heroUrl } from "@/lib/cloudinary"

export const nhaPhaPrison: Location = {
  slug: "nha-pha-historical-site",
  name: "Di tích Nhà Pha",
  updatedAt: "2026-09-21",
  provinces: ["quang-ngai"],
  destination: "ly-son",
  lat: 15.386246450441275,
  lng: 109.14170348441878,
  address: "Thôn Đông, An Hải, Lý Sơn, Quảng Ngãi",
  type: ["historic-site", "prison", "lighthouse", "history", "landmark"],
  categories: ["history", "coast"],
  experiences: ["history", "culture", "photography"],
  tags: [
    "🏚️ French Colonial Complex",
    "🚩 1945 Uprising Site",
    "🗼 Lý Sơn Lighthouse",
    "🇻🇳 Vietnamese Resistance History",
    "french-colonial-era",
  ],
  bestTime: "Sunrise or sunset from the lighthouse viewing platform for the best panoramic light over the island",
  bestMonths: [3, 4, 5, 6, 7, 8],
  entranceFee: "Free",
  openingHours: "The grounds are open freely, day and night, with no ticket gate or fixed hours. However, staffed access to climb the lighthouse itself appears to end around 4-5 PM based on visitor reports - plan a daytime visit if climbing to the top is your goal.",
  mapUrl: "https://www.google.com/maps?q=15.386246450441275,109.14170348441878",
  streetView: {embedUrl: "https://www.google.com/maps/embed?pb=!4v1775831997727!6m8!1m7!1sCAoSF0NJSE0wb2dLRUlDQWdJRDJncmkxekFF!2m2!1d15.38619140746176!2d109.1417389374112!3f33.89405502628472!4f30.03900252187273!5f0.7820865974627469", },
  heroImage: heroUrl("nha-pha-prison-ly-son-island-4_mbem6i"),
  gallery: [
    "nha-pha-prison-ly-son-island-6_jpwe1r",
    "nha-pha-prison-ly-son-island-4_mbem6i",
    "nha-pha-prison-ly-son-island-5_ficyrx",
    "nha-pha-prison-ly-son-island-3_jvnjfv",
    "nha-pha-prison-ly-son-island-2_gmgngi",
    "nha-pha-prison-ly-son-island-1_mptfur",
  ],
  seoDescription:
    "Di tích Nhà Pha is a French colonial complex in Đông village, An Hải commune, Lý Sơn, built in 1890 as an office building, lighthouse, and kitchen. The site witnessed the island's 1945 uprising, later became a wartime prison under both French and American administrations, and today combines revolutionary history with the still-functioning Lý Sơn lighthouse - one of the best panoramic viewpoints on the island.",
  tips: [
    "Nhà Pha is one site, not two: the same 1890 French complex includes the office building (now a historical relic) and the lighthouse (nhà đèn), which still operates and can be climbed",
    "The lighthouse has a 362-step interior spiral staircase up to viewing platforms with railings and telescopes - one of only three spots on the island (with the Thới Lới and Giếng Tiền craters) offering a full panoramic view of Lý Sơn, especially striking around sunrise, roughly 5:15-6:10 AM depending on the season",
    "Don't confuse this with the nearby Hải Đăng Mù Cu (Mù Cu Cape lighthouse), about 1km away - a separate structure with its own identity",
    "The grounds have no fixed hours and no ticket gate, but some visitors report being unable to climb the lighthouse itself after around 4-5 PM once staffed hours end - aim for a daytime visit if the climb is your priority",
    "Opening hours for climbing the lighthouse this early aren't confirmed, so you may not be able to go up right at sunrise - if that's the case, the nearby Hải Đăng Mù Cu, about 1km away, is a workable backup spot to watch the sunrise instead",
    "The former office building retains the only surviving French colonial architecture on the island - look for the 60cm-thick interior walls and underground water cisterns built into the floor for natural temperature regulation",
    "This is a low-key historical site with minimal on-site interpretation for the revolutionary history specifically - some background reading beforehand will make the visit more meaningful",
    "Combine with the Hải Đội Hoàng Sa museum and the two communal houses for a fuller picture of Lý Sơn's layered history",
    "Time needed: 45-60 minutes",
  ],
  content: {
    intro:
      "Di tích Nhà Pha is a French colonial complex built in 1890 in Đông village, An Hải commune, originally comprising an office building, a lighthouse (nhà đèn), and a kitchen. 'Nhà Pha' derives from the French 'phare' ('lighthouse'), reflecting the site's original role as a French lighthouse and administrative compound - not, as sometimes assumed, from 'maison de force'. After the French returned to Lý Sơn in 1951, the complex was also converted into a detention centre, giving the site its later association with imprisonment and resistance history. On August 16, 1945, as part of the nationwide August Revolution, Lý Sơn's residents rose up to seize power here, raising the red flag with the hammer and sickle at Nhà Pha itself. The French reoccupied the island in 1951 and turned the complex into a detention centre, imprisoning more than 120 people; roughly 100 more resistance fighters and suspected sympathisers were arrested and held in 1953. The United States later used Nhà Pha as its own base and detention site during the war, holding revolutionary figures including Võ Khế, Trần Quy, and Lê Thiện. The former office building survives as the only remaining example of French colonial architecture on the island, with interior walls up to 60cm thick and underground water cisterns beneath the floor for natural cooling and heating. The site was recognised as a provincial-level historical relic by Quảng Ngãi province in 2014.",
    howToGetThere:
      "Di tích Nhà Pha is in Đông village, An Hải commune, near the island's eastern side. Ask locally for precise directions, as the historical office building itself is not prominently signposted, though the lighthouse is a well-known local landmark.",
    whatToExpect:
      "The complex combines the quiet, under-interpreted remains of the former office building with the still-active lighthouse, which visitors can climb. Note that this is Hải Đăng Lý Sơn specifically, distinct from the nearby Hải Đăng Mù Cu (Mù Cu Cape lighthouse), about 1km away - the two are sometimes confused given how close they sit to each other. The French originally named the lighthouse 'Phare Polo Canton' (Sở Đèn Pha), built it in 1890 about 80m from the shoreline, and brought it into operation in 1898; it topped (and reportedly still tops) a list of Vietnam's five tallest lighthouses, and is cited among nine lighthouses particularly popular with independent travellers nationwide. Functionally, it helps fishing vessels navigate and find their bearings at sea, marks the coastline and dangerous shoals, guides safe entry into port, and can assist aircraft with orientation. The lighthouse itself has been rebuilt since its original 1890s iron structure: the old lamp was removed and briefly relocated to the summit of Núi Thới Lới between 1982-2000 before being judged unsuitable there, then rebuilt on the original site in 2000 and brought back into service on October 16, 2001, with a modern lamp manufactured in Spain. The current structure is a hexagonal cylinder about 3.6m in average diameter with flared support legs at the base, standing 45m from its foundation (50.4m to the sea-level datum), with a light range of 21 nautical miles. A 362-step interior spiral staircase leads up to viewing platforms with iron railings and telescopes, looking out over Lý Sơn's coastline, onion and garlic fields, and fishing boats - one of just three spots on the island (alongside the Thới Lới and Giếng Tiền volcanic craters) offering a full panoramic view. Sunrise is particularly striking here, typically falling somewhere between 5:15-6:10 AM depending on the season. The historical side of the site - the 1945 uprising and later imprisonments - has little on-site interpretation, so the significance is easy to miss without some background reading first.",
    travelTips:
      "Di tích Nhà Pha rewards visitors who come with a little background on the 1945 uprising and the site's later use as a prison under both French and American administrations - the physical remains alone don't convey much without that context. The lighthouse climb, on the other hand, is worthwhile purely for the panoramic views, regardless of interest in the history. Pair with both communal houses and the Hải Đội Hoàng Sa museum for a coherent half-day covering the island's past.",
  },
  insights: {
    highlights: [
      "A French colonial complex (1890) combining a historic office building, a kitchen, and the still-functioning Lý Sơn lighthouse",
      "One of Lý Sơn's most significant revolutionary sites - the location of the island's 1945 uprising and later wartime imprisonment under both French and American administrations",
      "A 362-step lighthouse climb to one of the island's three best panoramic viewpoints, especially striking at sunrise or sunset",
    ],
    thingsToKnow: {
      crowds: "The historical office building sees very few visitors; the lighthouse itself draws more visitors specifically for the views",
      difficulty: "The lighthouse involves a 362-step spiral climb - moderate effort, manageable for most visitors but not a quick stroll",
      safety: null,
      accessibility: "The lighthouse has railings and viewing platforms; the historical office building is open-air ruins with no formal ticketing or facilities",
      seasonal: null,
    },
    visitorTips: [
      "Read up on the August 16, 1945 uprising and the site's role in it before visiting, since there's little on-site interpretation for the historical side",
      "Climb the lighthouse for sunrise or sunset if photography or panoramic views are your priority",
      "Combine with the Hải Đội Hoàng Sa museum and communal houses for a fuller half-day of Lý Sơn's history",
    ],
    faq: [
      { question: "Is Nhà Pha the same as the Lý Sơn lighthouse?", answer: "Yes - they're part of the same 1890 French colonial complex, which originally included an office building, the lighthouse (nhà đèn), and a kitchen. The office building later became a wartime prison, giving the whole site its 'Nhà Pha' name, while the lighthouse has continued operating (with a rebuild in 2000-2001) as a separate function within the same site." },
      { question: "What was Nhà Pha used for historically?", answer: "Built in 1890 as a French administrative complex, it became the site of Lý Sơn's 1945 uprising during the August Revolution, then a French detention centre after 1951 (holding 120+ people), and later a US base and prison holding figures including Võ Khế, Trần Quy, and Lê Thiện." },
      { question: "Can I climb the lighthouse?", answer: "Yes - a 362-step interior spiral staircase leads to viewing platforms near the top, one of the island's three best panoramic viewpoints alongside the Thới Lới and Giếng Tiền craters." },
    ],
    sentiment: {
      positive: "Visitors climbing the lighthouse consistently praise the panoramic views, especially at sunrise or sunset. Those who research the historical side beforehand find the 1945 uprising and imprisonment history genuinely moving.",
      negative: "The historical office building has minimal on-site interpretation, so its significance is easy to miss without prior reading - some visitors focus only on the lighthouse views and leave unaware of the site's deeper history.",
    },
  },
}
