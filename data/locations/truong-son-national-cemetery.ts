import { Location } from "@/data/location"
import { heroUrl } from "@/lib/cloudinary"

export const truongSonNationalCemetery: Location = {
  slug: "truong-son-national-cemetery",
  name: "Truong Son National Cemetery",
  updatedAt: "2026-09-22",
  provinces: ["quang-tri"],
  destination: "",
  lat: 16.958242841270042,
  lng: 106.95473875403596,
  address: "Vĩnh Trường, Gio Linh, Quảng Trị",
  type: ["historic-site", "landmark"],
  categories: ["history"],
  experiences: ["history", "culture", "photography", "walking-tour"],
  tags: ["🪦 War Memorial", "🕊️ Ho Chi Minh Trail", "📸 Sobering Visit", "🇻🇳 National Memorial", "vietnam-war"],
  bestTime: "Morning, for cooler temperatures and better light for photography",
  bestMonths: [1, 2, 3, 4, 11, 12],
  entranceFee: "Free",
  openingHours: "7:00 AM – 5:00 PM",
  mapUrl: "https://www.google.com/maps?q=16.958242841270042,106.95473875403596",
  streetView: {embedUrl: "https://www.google.com/maps/embed?pb=!4v1775891559569!6m8!1m7!1sCAoSHENJQUJJaEJCZl9FS2FabFg2bG9ueG9sUDI3QWs.!2m2!1d16.95598295880135!2d106.9525472328111!3f247.77729910758154!4f-28.576223232009227!5f0.5495649665008955", },
  heroImage: heroUrl("truong-son-national-cemetery_t3kr9p"),
  gallery: [
    "truong-son-national-cemetery-2_ehruk7",
    "truong-son-national-cemetery-5_htybto",
    "truong-son-national-cemetery_t3kr9p",
    "truong-son-national-cemetery-3_imouqe",
    "truong-son-national-cemetery-4_igduoh",
  ],
  seoDescription:
    "Truong Son National Cemetery, on Bến Tắt Hill in Quảng Trị, is the largest war cemetery in Vietnam, holding over 10,000 graves of soldiers who died on the Ho Chi Minh Trail. Built 1975-1977 across three hills near the former DMZ, each provincial section has its own distinctly-styled memorial house, making it one of the most affecting and architecturally layered war memorials in Southeast Asia.",
  tips: [
    "Time needed: most visitors spend 1-2 hours here; the site is large (around 140,000 sqm across three hills and 10 sections), so budget more time if you want to walk multiple provincial sections rather than just the central monument area",
    "Dress respectfully - covered shoulders and knees are appropriate given the site's significance. Behaviour should match the solemnity of the location.",
    "The cemetery is best visited as part of a DMZ day tour from Hue or Dong Ha, combined with Vinh Moc Tunnels and the Hien Luong Bridge - the historical context of multiple sites together is more powerful than any single stop",
    "The central monument, on a hill roughly 32m above the entrance, features a distinctive hollow, three-sided open design in white stone, surrounded by six stone relief carvings depicting the different military units of the Trường Sơn forces - Vietnamese visitors typically pay respects here with incense before walking the wider cemetery",
    "Each provincial section has its own memorial house, built in a style reflecting that province's own local architecture and cultural symbols - worth slowing down to notice as you move between sections rather than just walking past",
    "Behind the central monument stands a Bodhi tree that's said to have grown there on its own decades ago - a quietly symbolic detail many visitors pause at",
    "There's a large bell at a bell tower on site that visitors can ring, inscribed with a commemorative verse - a common small ritual during a visit",
    "Visit in the morning when the light is better for photography and before tour groups from Hue arrive around 10:00 AM",
    "If you want help with incense-offering procedures, registering to pay respects at the main monument, or a guided explanation, come during staffed hours: roughly 7:00-11:30 AM or 1:30-5:00 PM, when caretaker staff are present at the reception house",
    "A natural route through the site: reception house at the gate, incense offering at the central monument, then the provincial grave sections, ending at the landscaped lake area",
    "There are very few food stalls in the immediate area - bring your own food and water, or plan to eat back in town. Flower and incense vendors are available right outside the entrance gate if you didn't bring your own",
    "The entrance road is scenic and open but shares space with a fair number of trucks - riding by motorbike can get dusty",
    "If you'd like to offer incense at a quieter, more remote section on higher ground, ask the caretaker staff for permission - a thoughtful way to pay respects to graves that see fewer visitors",
    "Some visitors following folk tradition avoid visiting right at 12 noon (giờ chính Ngọ), a time some consider inauspicious for this kind of visit - purely a matter of personal belief, not an official rule",
  ],
  content: {
    intro:
      "Truong Son National Cemetery sits on Bến Tắt Hill (đồi Bến Tắt) near Cồn Tiên in Gio Linh district of Quảng Trị province, close to the former Demilitarized Zone that divided North and South Vietnam along the 17th parallel, and named for a nearby crossing on the Bến Hải river. The site's location isn't incidental: Bến Tắt sits near Khe Hó, where the 559th Division - the unit responsible for building and running the Ho Chi Minh Trail - was founded back in 1959, tying the cemetery directly to the origin point of the trail it commemorates. Construction began on October 24, 1975 - just months after reunification - and was completed on April 10, 1977, built by the 559th Division's engineering command with the help of more than 40 army units. The cemetery holds 10,333 graves of Vietnamese People's Army soldiers and volunteer youth who died on the Ho Chi Minh Trail logistics network that ran through the Trường Sơn mountain range during the American War. Many of these graves remain unidentified, though the exact number changes as identification efforts continue; where a soldier's identity couldn't be recovered, the headstone reads simply 'Liệt sĩ chưa biết tên, quê quán: Tổ quốc' - 'An unknown soldier, homeland: the Fatherland' - one of the most quietly affecting details for visitors who read the markers closely. Separately, a nearby memorial temple commemorates the far larger number of Trường Sơn soldiers - by some accounts over 13,000 - whose remains were never recovered at all and who have no grave here or elsewhere. Spread across three hills and roughly 140,000 square metres, the site is organised into 10 main sections, historically grouped by home province using the province names in use at the time of construction (Hà Nội, Nghệ Tĩnh, Thanh Hóa, Bình Trị Thiên, Hà Nam Ninh, and others, some of which have since been split or renamed); distinctively, each provincial section has its own memorial house, built in a style reflecting that province's own local architecture and cultural symbols, so the cemetery reads as much as a map of the whole country's wartime sacrifice as a single uniform site. Each white grave marker carries the soldier's name, date of birth, date of death, and home district - a level of individual record that reflects the sustained effort to identify the dead along a trail where many casualties occurred in remote forest far from medical facilities.",
    howToGetThere:
      "Truong Son National Cemetery is roughly 30-38km northwest of Đông Hà city (about 32km along Highway 15/the Ho Chi Minh Trail road before turning off at the signposted junction) and about 83km north of Huế. Most visitors reach it as part of an organised DMZ day tour from Huế, which typically combines the cemetery with Vinh Moc Tunnels, the Hien Luong Bridge at the former border, and the Khe Sanh Combat Base. By motorbike or car from Đông Hà, the cemetery is approximately 40-60 minutes via Highway 15, which follows part of the Ho Chi Minh Trail's original route. The site is well signposted from the main roads, with a car park at the entrance.",
    whatToExpect:
      "The entrance gate opens onto a wide central avenue lined with pine trees leading toward the central monument area on Bến Tắt Hill's summit, about 32m above the entrance. The monument itself is a distinctive hollow, three-sided open structure in white stone, surrounded by six stone relief carvings depicting the different military units of the Trường Sơn forces alongside a separate Tổ Quốc Ghi Công (Fatherland Remembers) statue group; a Bodhi tree said to have grown there naturally stands just behind it, and a large bell at a nearby bell tower is inscribed with a commemorative verse for visitors to ring. Beyond the monument, the hillsides are covered in rows of white grave markers extending across the cemetery's 10 sections, each grouped by home province and marked by its own distinctly-styled memorial house reflecting that province's local architecture. A separate memorial temple nearby, Đền Tưởng Niệm Liệt Sĩ Trường Sơn - Bến Tắt, is dedicated specifically to soldiers whose remains were never recovered, extending the site's memorial space beyond the marked graves. An on-site lookup system, accessible via website and QR codes, helps visitors search for specific soldiers' records. Walking the rows is a slow and particular experience - the dates of death cluster in years of heavy American bombing campaigns, and the home provinces on the markers trace the geography of wartime mobilisation from across the country. The site is maintained in good condition, with paved or cemented paths, mown grass, and trees and flowers throughout that keep it from feeling cold or bleak.",
    travelTips:
      "Quảng Trị province holds more war memorials and historical sites per square kilometre than anywhere else in Vietnam - the province was the most intensively bombed area in the country and the site of some of the war's most destructive ground campaigns. Truong Son Cemetery is the largest and most formally significant of these sites, but the broader DMZ landscape repays a full day of exploration. The nearby Vinh Moc Tunnels - where an entire village lived underground for years to survive the bombing - provides a civilian counterpoint to the military focus of the cemetery. For travellers with a serious interest in the war's history, staying a night in Đông Hà rather than doing the DMZ as a day trip from Huế allows for a less rushed engagement with the sites.",
  },
  insights: {
    highlights: [
      "The final resting place of over 10,000 soldiers who died on the Ho Chi Minh Trail, on Bến Tắt Hill in Quảng Trị",
      "A cemetery spread across three hills and 10 provincial sections, each with its own distinctly-styled memorial house reflecting local architecture from that province",
      "A central monument built 1975-1977 with a distinctive hollow, three-sided design and six stone relief carvings depicting the Trường Sơn forces' various military units",
    ],
    thingsToKnow: {
      crowds: "Attracts large numbers of visitors annually, especially around July 27 (War Invalids and Martyrs Day)",
      difficulty: null,
      safety: null,
      accessibility: "The cemetery spans a large area across three hills; most visitors cover the central monument and a few provincial sections in 1-2 hours, though seeing every section in detail would take considerably longer",
      seasonal: "Visit during cooler mornings or late afternoons; bring light rain gear as weather can be unpredictable",
    },
    visitorTips: [
      "Bring flowers or incense to pay respects; some graves are memorial markers without remains, given the conditions many soldiers died under",
      "Slow down at a few different provincial sections rather than only the central monument - each has its own distinct memorial house style worth noticing",
      "Visit with a contemplative mindset; maintain a quiet, respectful demeanour rather than treating it as a typical tourist attraction",
    ],
    faq: [
      { question: "How many soldiers are buried here?", answer: "10,333 graves, primarily those who died on the Ho Chi Minh Trail. Many remain unidentified, while ongoing identification efforts continue to recover names and personal information." },
      { question: "How long does it take to visit?", answer: "Most visitors spend 1-2 hours covering the central monument and a few provincial sections. The site is large - spread across three hills and 10 sections - so seeing everything in detail would take considerably longer." },
      { question: "When is the best time to visit?", answer: "Morning is best for cooler temperatures and better light. July 27 (War Invalids and Martyrs Day) draws large crowds of Vietnamese visitors specifically to pay respects. Bring rain protection, as weather in the area can be unpredictable." },
    ],
    sentiment: {
      positive: "Visitors find the cemetery deeply moving and solemn, with well-maintained grounds, distinctive province-by-province architecture, and details like the central monument and bell tower that inspire gratitude and respect for wartime sacrifice.",
      negative: null,
    },
  },
}
