import { Location } from "@/data/location"
import { heroUrl } from "@/lib/cloudinary"

export const minhDamMountain: Location = {
  slug: "minh-dam-mountain",
  name: "Minh Đạm Mountain",
  updatedAt: "2026-09-21",
  provinces: ["vung-tau"],
  destination: "",
  lat: 10.406176641081109,
  lng: 107.2592560602553,
  address: "Phước Hải, Đất Đỏ, Bà Rịa - Vũng Tàu",
  type: ["mountain", "nature", "history"],
  categories: ["nature", "history"],
  experiences: ["trekking", "history", "photography", "caving", "hiking"],
  tags: ["⛰️ Resistance Base", "🥾 Forest Trek", "🏛️ National Historical Relic", "🐒 Wild Monkeys"],
  bestTime: "Roughly 7-11 AM, when the sun isn't too harsh - afternoons turn overcast and make the caves harder to navigate",
  bestMonths: [11, 12, 1, 2, 3, 4, 5],
  entranceFee: "Free",
  openingHours: "7:00 AM - 5:00 PM",
  mapUrl: "https://www.google.com/maps?q=10.406176641081109,107.2592560602553",
  streetView: {embedUrl: "https://www.google.com/maps/embed?pb=!4v1775829220039!6m8!1m7!1sCAoSFkNJSE0wb2dLRUlDQWdJRHFqYWVQR3c.!2m2!1d10.40804188429981!2d107.2624138697891!3f114.65771270226824!4f-9.640437665838249!5f0.7820865974627469", },
  heroImage: heroUrl("minh-dam-mountain-vung-tau-9_jiqr27"),
  gallery: [
    "minh-dam-mountain-vung-tau-4_b0weij",
    "minh-dam-mountain-vung-tau-5_xsmthg",
    "minh-dam-mountain-vung-tau-9_jiqr27",
    "minh-dam-mountain-vung-tau-7_myodtx",
    "minh-dam-mountain-vung-tau-2_k1xprl",
    "minh-dam-mountain-vung-tau-6_psrqoc",
    "minh-dam-mountain-vung-tau-3_jfx0wa",
    "minh-dam-mountain-vung-tau-8_hvow9q",
    "minh-dam-mountain-vung-tau_rvofhy",
  ],
  seoDescription:
    "Minh Đạm Mountain is a 355m, 8km-long forested range in Bà Rịa-Vũng Tàu with three sides facing the sea, over 300 caves, and a resistance base recognised as a National Historical Relic in 1993. Named for two soldiers who died defending it in 1948, it offers forest trekking, cave exploration, and coastal views near Long Hải beach.",
  tips: [
    "⚠️ Don't rely on a generic Google Maps pin - search specifically for 'Minh Dam Historic Relic' to get accurate directions to the actual entrance",
    "Time needed: 1-3 hours for a beginner-friendly route; longer if exploring deeper trails",
    "For a first visit, the safest and easiest route is to Bạch Vân Điện: from the parking area, follow the gravel road on the left of the site entrance, then the small concrete path to a 3-way junction, turn left, and you'll reach Bạch Vân Điện with a large rock outcrop for photos - roughly 15-20 minutes of climbing",
    "Two other reasonable beginner routes: the Đá Chẻ flagpole (stairs beside the parking area, then the main path, about 30+ minutes to the top) and Tứ Sơn Tự temple (gravel road on the left of the entrance, about 45 minutes)",
    "Only follow major trails, cement paths, or paths marked with rope/cord - if a path matches none of these three, don't go in. There's a documented case of a visitor getting lost in the deeper trails near Bạch Vân Động and needing help from local rangers",
    "The deeper network beyond Bạch Vân Động (toward Tứ Sơn Tự, Bồng Lai, Hàm Rồng, and Hang Dơi/Bat Cave) has many interconnecting junctions that are easy to confuse - only attempt this in a group, prioritise official signage over red rope markers, and don't go if you lack experience or a proper tracklog",
    "If you do get turned around, stay calm and sit tight - the mountain is small and not very tall, so waiting for help is a reasonable option rather than pushing on uncertain",
    "Wild monkeys are present and can be assertive around food - keep food and drinks secured and avoid provoking them",
    "Bring insect repellent, water, and light snacks - services on the mountain itself are minimal",
    "The road up is steep and winding with occasional large vehicles coming the other way - drive carefully",
  ],
  content: {
    intro:
      "Minh Đạm is a 355m-high, 8km-long mountain range in Bà Rịa-Vũng Tàu, running through several communes including Tam Phước, Long Hải town, Phước Hưng, Phước Hải, and Long Mỹ, with three sides facing the sea. The range is actually made up of several distinct peaks - including Núi Đá Dựng, Hòn Thung, Núi Đá Ngang, Núi Điện Bà, Núi Châu Viên, and Núi Trương Phi (also called Núi Kỳ Vân and Thùy Vân) - collectively forming what's known as the Châu Viên and Châu Long ranges. It's roughly 6km northeast of Long Hải town and about 30km from central Vũng Tàu. The mountain's rugged terrain and more than 300 caves of various sizes made it a natural stronghold for Vietnamese resistance fighters during both the French and American wars, organised into four historic base areas - Đá Chẻ, Đá Chồng, Chùa Viên, and Chùa Giếng Gạch - connected today by a scenic stone-paved forest path. Specific caves still preserved include the District Party Committee cave (Hang Huyện Ủy), the Ordnance cave (Hang Quân Giới), and the Medical cave (Hang Quân Y), among others, several retaining original furniture, wooden bridges, and cooking equipment from the resistance period. The mountain's name honours two soldiers, Mạc Thanh Đạm and Bùi Công Minh, who died defending the base on November 17, 1948 - local residents combined their names to name the mountain in their memory. The base was recognised as a National Historical Relic by the Ministry of Culture, Sports and Tourism in 1993. Beyond its history, Minh Đạm's forest supports rare hardwoods like gõ đỏ and cẩm lai, medicinal plants, and wildlife, drawing hikers as much for the trekking as for the history.",
    howToGetThere:
      "Minh Đạm Mountain is located in Phước Hải commune, Đất Đỏ district, approximately 100km from Ho Chi Minh City and 30km from Vũng Tàu city. By car or motorbike from HCMC, follow Highway 51 toward Vũng Tàu then branch south toward Long Hải and Phước Hải - the journey takes around 2 hours. From Vũng Tàu, follow road ĐT44A through Phước Hải and Đất Đỏ toward Long Hải; at the Pháp Hoa Pagoda junction (ngã ba chùa Pháp Hoa), turn left to reach the site. Don't rely on a generic Google Maps pin - search specifically for 'Minh Dam Historic Relic' for accurate directions. The final approach road is steep and winding, with occasional large vehicles coming the other way, so drive with care. Safe parking is available at the base near the historical site entrance.",
    whatToExpect:
      "From the parking area, a beginner-friendly route leads to Bạch Vân Điện (about 15-20 minutes), where stairs descend to a large rock outcrop with sea views - a popular, manageable first stop. Two other reasonable routes for newer visitors lead to the Đá Chẻ flagpole (roughly 30+ minutes via stairs and the main path) or Tứ Sơn Tự temple (about 45 minutes via a gravel road). Deeper trails connect Bạch Vân Động to Bồng Lai, Hàm Rồng, and Hang Dơi (Bat Cave), but this network has many confusing junctions and isn't recommended without a group and real trekking experience - there's a documented case of a visitor getting lost here and needing local rangers' help. The historical zones themselves are linked by an attractive stone-paved forest path, with preserved furniture, wooden bridges, and cooking equipment giving a sense of how resistance fighters lived - some sites even demonstrate cooking bamboo-tube rice or making fire from stone. Wild monkeys are common and can be assertive around food. The overall climb is moderate and suitable for most fitness levels on the main routes, though afternoons turn overcast and make the cave sections harder to navigate - mornings are notably better.",
    travelTips:
      "Minh Đạm works best as a secondary stop alongside Long Hải beach rather than a full day trip on its own - many visitors treat it as a morning activity on day two of a Long Hải overnight stay, after breakfast and before heading back. If you're coming from HCMC as a single day trip, decide your priority: a beach-focused day doesn't leave much room for the mountain, while a history-and-nature-focused day can make room for a shorter beach stop instead. Families with older, capable kids and couples or groups who've already done the beach and want something different tend to get the most out of it; families with young children or elderly members with limited walking stamina may want to treat it as optional rather than essential, especially in hot weather. Whatever your pace, this is a historical site as much as a hike - keep noise down, don't litter or write on structures, and take the time to actually absorb the place rather than rushing through for photos.",
  },
  insights: {
    highlights: [
      "A 355m, 8km-long mountain range with three sides facing the sea and over 300 resistance-era caves",
      "A National Historical Relic (since 1993) named after two soldiers who died defending it in 1948",
      "Several beginner-friendly routes (Bạch Vân Điện, Đá Chẻ, Tứ Sơn Tự) alongside a more complex, easy-to-get-lost deeper trail network",
    ],
    thingsToKnow: {
      crowds: null,
      difficulty: "Beginner-friendly on the main routes (Bạch Vân Điện, Đá Chẻ, Tứ Sơn Tự); the deeper trail network beyond Bạch Vân Động is confusing and better suited to experienced groups",
      safety: "Monkeys in the area can be assertive around food - keep food secured. Only follow major trails, cement paths, or rope-marked routes; a visitor has gotten lost in the deeper trails and needed local rangers' help. If lost, stay calm and wait, since the mountain is small.",
      accessibility: "Vehicles can park safely at the base near the historical site entrance; routes from there are on foot. Search 'Minh Dam Historic Relic' on Google Maps rather than trusting a generic pin.",
      seasonal: "Dry season (roughly November-May) is best for stable weather; rainy season (June-October) makes trails slippery and more hazardous.",
    },
    visitorTips: [
      "Search 'Minh Dam Historic Relic' on Google Maps for accurate directions, rather than a generic pin",
      "Stick to the Bạch Vân Điện, Đá Chẻ, or Tứ Sơn Tự routes if you're new to the site; only attempt deeper trails in a group with real trekking experience",
      "Bring insect repellent, water, and snacks, and secure any food against monkeys",
    ],
    faq: [
      { question: "Which route should a first-time visitor take?", answer: "Bạch Vân Điện is the easiest and safest option - about 15-20 minutes from the parking area to a scenic rock outcrop. The Đá Chẻ flagpole (30+ minutes) and Tứ Sơn Tự temple (about 45 minutes) are also reasonable for beginners." },
      { question: "Is it easy to get lost on Minh Đạm?", answer: "On the main routes, no - they're well-trodden and signed. The deeper trail network beyond Bạch Vân Động (toward Bồng Lai, Hàm Rồng, and Hang Dơi) has many confusing junctions, and there's a documented case of a visitor getting lost there and needing local rangers' help. Stick to major trails, cement paths, or rope-marked routes, and only go deeper in an experienced group." },
      { question: "How did the mountain get its name?", answer: "It's named for two soldiers, Mạc Thanh Đạm and Bùi Công Minh, who died defending the resistance base here on November 17, 1948. Local residents combined their names to honour them." },
    ],
    sentiment: {
      positive: "Visitors praise the combination of genuine history and natural scenery, the manageable main routes, and the sense of stepping into a quieter, greener side of the Long Hải area.",
      negative: "Some visitors report confusing trail junctions deeper into the mountain, aggressive monkeys around food, and inconsistent signage away from the main routes.",
    },
  },
}
