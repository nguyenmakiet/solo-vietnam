import { Location } from "@/data/location"
import { heroUrl } from "@/lib/cloudinary"

export const aPaChai: Location = {
  slug: "a-pa-chai",
  name: "A Pa Chai (Vietnam's Westernmost Point)",
  updatedAt: "2026-08-24",
  provinces: ["dien-bien"],
  destination: "",
  lat: 22.401192335843305,
  lng: 102.14371962758165,
  address: "Sín Thầu, Mường Nhé, Điện Biên",
  type: ["landmark", "mountain", "nature"],
  categories: [],
  experiences: ["trekking", "photography", "motorcycling", "culture"],
  tags: ["🌐 Westernmost Point", "🏔️ Triple Border", "🥾 Remote Trek", "📸 Photography"],
  bestTime: "Oct - Apr (dry season; wet season May - Sep makes the trail muddy and dangerous underfoot); Sep-Oct for golden rice terraces en route, Nov-Dec for wild sunflower (dã quỳ), Jan-Mar for plum, peach, and bauhinia (hoa ban) blossoms along the Northwest highlands",
  bestMonths: [1, 2, 3, 4, 9, 10, 11, 12],
  entranceFee: "Free to view the marker, but budget for local logistics: escort/guide fee around 400,000 VND/group, xe ôm around 500,000 VND/group, or motorbike rental around 300,000 VND/bike - registration with ID required at Đồn Biên Phòng 317 (A Pa Chải border post)",
  openingHours: "Arrive at Đồn Biên Phòng 317 before 4:00 PM - the border post does not receive visitors after that time",
  mapUrl: "https://www.google.com/maps?q=22.401192335843305,102.14371962758165",
  streetView: {embedUrl: "https://www.google.com/maps/embed?pb=!4v1775920934981!6m8!1m7!1sCAoSFkNJSE0wb2dLRUlDQWdJQ3ZsYk9oRmc.!2m2!1d22.40066539062425!2d102.1439166707527!3f12.022172178329424!4f-12.010206953621775!5f0.7820865974627469", },
  heroImage: heroUrl("a-pa-chai-dien-bien-3_cezsa3"),
  gallery: [
    "a-pa-chai-dien-bien-4_mieuhy",
    "a-pa-chai-dien-bien_p39kld",
    "a-pa-chai-dien-bien-2_zopyc0",
    "apachai-dien-bien-5_inakpo",
    "apachai-dien-bien-6_nm09io",
  ],
  seoDescription:
    "A Pa Chai is Vietnam's westernmost point - a remote peak in Điện Biên province where the borders of Vietnam, Laos, and China meet at a tri-border marker, reached via Đồn Biên Phòng 317 in Mường Nhé district's Sín Thầu commune.",
  tips: [
    "Important: a large flagpole has recently been built near the trailhead, but it is NOT the tri-border marker - the actual marker is about 2.4km further on. Don't mistake the flagpole for the destination",
    "Registration happens at Đồn Biên Phòng 317 (the A Pa Chải border post) - bring valid ID (CCCD or passport), and you must arrive before 4:00 PM, since the post doesn't receive visitors after that. A border guard soldier escorts your group to the marker, both to guide you and to make sure no one wanders across the border by accident",
    "Foreign visitors face a more complex process than Vietnamese citizens - foreigners working for a central government agency need a permit from the Ministry of Public Security, while foreigners residing locally need one from the provincial police where they're registered. Contact a homestay or guesthouse near Mường Nhé or Sín Thầu in advance to ask about current requirements, since this can be genuinely difficult to navigate without local help",
    "As of a May 2026 update, motorbikes can ride to within about 200-300m of the actual marker, cutting the walk down considerably - though the newly extended stretch of road is still under construction and can be rough with loose earth and debris. Expect this to keep changing as work continues",
    "Costs to budget beyond the ticket-free marker itself: an escort/guide fee (~400,000 VND/group), a xe ôm (~500,000 VND/group), or motorbike rental (~300,000 VND/bike) - confirm current rates locally, as these appear to shift over time",
    "Cell phone signal can be unreliable this close to the border, and some phones may briefly pick up a Chinese network - worth knowing so you're not caught off guard by unexpected roaming charges or dropped connections",
    "Avoid Vietnamese public holidays (30 April, 2 September) - the border post's capacity to host, feed, and guide visitors is limited, and a crowded marker makes it hard to get a clean photo",
    "Take your rubbish back out with you if it won't burn - waste at the site is typically piled up and burned locally due to a lack of other disposal options, so anything that can't be burned is best carried back to a town with proper waste management",
    "A Pa Chải means 'flat, wide land' in the Hà Nhì language - fitting for a place otherwise defined by remote, rugged terrain",
    "Sín Thầu commune is home to seven ethnic groups, with the Hà Nhì making up about 98% of the population - one of the smallest and least-known highland groups in Vietnam, with a distinct culture and architecture visible in the village",
  ],
  content: {
    intro:
      "A Pa Chai marks the westernmost point of Vietnam - the peak in Mường Nhé district, Điện Biên province, where Vietnam, Laos, and China converge at a tri-border marker set atop Núi Khoang La San. The point sits at an elevation of 1,864 metres in a landscape of dense mountain forest populated by the Hà Nhì ethnic minority, one of the smallest highland groups in Vietnam - 'A Pa Chải' itself means 'flat, wide land' in the Hà Nhì language. The granite marker - a triangular pillar about 2 metres tall on a 5x5 metre base, with three faces pointing toward each of the three countries - was completed on 27 June 2005 through a joint agreement between all three governments, and Đồn Biên Phòng 317 was established in 2007 to manage the surrounding border area, including the marker itself.\n\nReaching A Pa Chai requires a combination of remote road travel to the district capital of Mường Nhé, a further journey to the Hà Nhì village of Sín Thầu, and registration at Đồn Biên Phòng 317 before the final approach. The journey used to be considerably harder - before border patrol roads were built, the climb from the base involved 4-5 hours on foot through sharp elephant grass tall enough to cut skin. Patrol road construction has steadily cut this down, and as of a May 2026 update, motorbikes can now ride to within roughly 200-300m of the marker, though this newly extended stretch is still rough and under construction. A large flagpole has also been erected near the trailhead in recent years - worth knowing that this is not the tri-border marker itself, which sits about 2.4km further along.",
    howToGetThere:
      "A Pa Chai is located in Sín Thầu commune, Mường Nhé district, approximately 250-260km west of Điện Biên Phủ city - reachable but genuinely remote, with the full one-way drive taking around 10 hours on rough roads. From Hanoi, overnight buses run from Mỹ Đình station to Điện Biên Phủ (10-12 hours, departing roughly 6-10 PM, 300,000-450,000 VND); a direct Hanoi-Mường Nhé bus exists but runs infrequently, so an overnight stop in Điện Biên Phủ before continuing is the more practical option for most visitors. From Điện Biên Phủ, rent a high-clearance car or motorbike and follow QL12 through Mường Chà, then Tỉnh lộ 131 through Chà Cang to Sín Thầu - allow 5-6 hours for this stretch. From Sín Thầu, register at Đồn Biên Phòng 317 before continuing; as of May 2026 motorbikes can ride most of the remaining distance, leaving a final 200-300m on foot, though check current road conditions locally since this section is still being finished. The total journey from Điện Biên Phủ is realistically a minimum of 2-3 days once travel time, registration, and the return trip are accounted for.",
    whatToExpect:
      "At Đồn Biên Phòng 317, registration is straightforward for Vietnamese citizens with valid ID; a border guard soldier then escorts your group the rest of the way, both to guide you and to make sure no one strays across the border by mistake. Along the way, a large flagpole near the trailhead is easy to mistake for the destination, but the actual tri-border marker is about 2.4km further on. The marker itself is a granite pillar on a paved 5x5m base, its three faces pointing toward Vietnam, Laos, and China, each engraved with that country's national emblem and name - a striking, simple monument on a forest ridge with sweeping views into Laos and China on clear days. The Hà Nhì village of Sín Thầu along the way is a significant cultural experience in itself - traditional architecture and daily life that few outside visitors ever encounter. Depending on the latest road conditions, expect anywhere from a steep, roughly hour-long climb (with a faster descent) to a much shorter final walk of a few hundred metres, given how quickly the access road has been improving. Allow about 3-4 hours round trip from Đồn Biên Phòng 317 to the marker and back.",
    travelTips:
      "A Pa Chai is for travellers who specifically want to reach one of Vietnam's geographic extremities and are prepared for the logistics involved - multiple days of remote road travel, a border post registration process, and genuine physical distance from any city or major town. Foreign visitors should expect extra steps and should reach out to local homestays or guesthouses ahead of time to understand current requirements, since procedures for non-Vietnamese citizens are noticeably more involved. It's not a casual add-on to a Điện Biên itinerary - treat it as the primary reason for visiting Mường Nhé district, and plan for 2-3 days minimum from Điện Biên Phủ there and back. Time your trip with one of the Northwest's seasonal highlights if you can (golden rice terraces in Sep-Oct, wild sunflower in Nov-Dec, plum, peach, or bauhinia blossoms Jan-Mar) and consider combining the trip with Mù Cang Chải, Sa Pa, or Y Tý for a broader Northwest loop, since you'll already be deep in the region. The reward is proportional to the effort: standing at the point where three countries' borders meet, in a part of Vietnam that sees a fraction of the visitors of almost anywhere else in the country. Visitors must stay on the Vietnamese side of the marker and follow the escorting border guards' instructions at all times - this is a working border post, not a place to step across into Laos or China.",
  },
  insights: {
    highlights: [
      "One of Vietnam's four extreme geographic points, and the only one requiring a formal border-post registration process rather than just a long drive",
      "A working tri-border marker rather than a tourist monument - reached with a military escort, on a site that's still actively used by border guards",
      "The Hà Nhì village of Sín Thầu along the way, home to one of Vietnam's smallest and least-visited ethnic minority communities",
    ],
    thingsToKnow: {
      crowds: "Very few visitors compared to almost anywhere else in Vietnam, given the distance and logistics involved. Vietnamese public holidays (30 April, 2 September) see a relative spike that can strain the border post's capacity to host and guide visitors.",
      difficulty: "Moderate to strenuous depending on current road conditions - historically a multi-hour climb, though recent road improvements have cut the walking portion down considerably. Expect this to keep changing.",
      safety: null,
      accessibility: "Requires a high-clearance vehicle or motorbike for much of the journey; the final stretch to the marker has varied over time between a serious hike and a short walk, depending on ongoing road construction.",
      seasonal: "Dry season (Oct-Apr) is far safer and more reliable for the roads and trail. Wet season (May-Sep) brings a real risk of impassable roads and a muddy, dangerous trail.",
    },
    visitorTips: [
      "Confirm current registration requirements and road conditions locally before setting out - both have changed significantly in recent years and can keep changing",
      "Arrive at Đồn Biên Phòng 317 well before 4:00 PM to guarantee entry",
      "Don't mistake the large flagpole near the trailhead for the tri-border marker itself - the real marker is about 2.4km further on",
    ],
    faq: [
      { question: "Can foreigners visit A Pa Chải?", answer: "Yes, but the process is more involved than for Vietnamese citizens. Foreigners working for a central government agency need a permit from the Ministry of Public Security; foreigners residing locally need one from the provincial police where they're registered. Contact a homestay or guesthouse near Mường Nhé or Sín Thầu ahead of time to confirm current requirements." },
      { question: "How difficult is the trek to the marker?", answer: "This has changed a lot as roads have improved. Historically it involved several hours of hiking through rough terrain; as of a May 2026 update, motorbikes can ride to within roughly 200-300m of the marker, though this newly extended road is still rough and under construction. Confirm current conditions locally before you go." },
      { question: "Do I need a guide?", answer: "Yes, effectively - after registering at Đồn Biên Phòng 317, a border guard soldier escorts your group to the marker. This isn't optional; it's both a safety measure and a way to make sure no one strays across the border by accident." },
      { question: "Can I stand in all three countries at once?", answer: "No. The marker sits on Vietnamese territory and visitors must stay on the Vietnamese side at all times, following the escorting border guards' instructions. It's a working border post, not a place to step across into Laos or China." },
      { question: "How long should I plan for the whole trip?", answer: "Realistically a minimum of 2-3 days round trip from Điện Biên Phủ, once you account for the long drive to Sín Thầu, registration, the visit itself, and the return journey. The visit to the marker and back from Đồn Biên Phòng 317 alone takes about 3-4 hours." },
      { question: "Can I visit A Pa Chải in one day?", answer: "Not realistically from Điện Biên Phủ - the round trip alone is roughly 500km of rough roads on top of registration and the walk or ride to the marker. Budget at least 2-3 days total." },
    ],
    sentiment: {
      positive: "No review data collected yet.",
      negative: "No review data collected yet.",
    },
  },
}
