import { Location } from "@/data/location"
import { heroUrl } from "@/lib/cloudinary"

export const imperialCitadelOfThangLong: Location = {
  slug: "imperial-citadel-of-thang-long",
  name: "Imperial Citadel of Thang Long",
  updatedAt: "2026-08-24",
  provinces: ["ha-noi"],
  destination: "ha-noi",
  lat: "21.032608",
  lng: "105.839832",
  address: "19C Hoàng Diệu, Ba Đình, Hà Nội",
  type: ["citadel", "heritage", "history", "museum"],
  categories: [],
  experiences: ["history", "culture", "photography", "walking-tour"],
  tags: ["🏯 UNESCO World Heritage", "👑 1,000 Years of Imperial History", "🌙 Night Tour", "🔍 Archaeological Site"],
  bestTime: "Oct - Apr (cool dry season; avoid public holidays when crowds are largest)",
  bestMonths: [10, 11, 12, 1, 2, 3, 4],
  entranceFee: "30,000 VND adults / 15,000 VND students. Night tour: 300,000 VND adults / 150,000 VND children under 12 / free for children under 5",
  openingHours: "8:00 AM - 5:00 PM daily. Night tour: Every Friday and Saturday at 7:00 PM (1.5 hours)",
  mapUrl: "https://www.google.com/maps?q=21.032608,105.839832",
  streetView: { embedUrl: "" },
  heroImage: heroUrl("imperial-citadel-of-thang-long-ha-noi_ktosjq"),
  gallery: [
    "imperial-citadel-of-thang-long-ha-noi_ktosjq",
    "imperial-citadel-of-thang-long-ha-noi-2_r29vcb",
    "imperial-citadel-of-thang-long-ha-noi-3_ckbdqp",
    "imperial-citadel-of-thang-long-ha-noi-4_kbro6x",
    "imperial-citadel-of-thang-long-ha-noi-5_nddbdu",
    "imperial-citadel-of-thang-long-ha-noi-6_qrmauw",
    "imperial-citadel-of-thang-long-ha-noi-7_lb6qve",
    "imperial-citadel-of-thang-long-ha-noi-8_jb9uul",
    "imperial-citadel-of-thang-long-ha-noi-9_alws3f",
  ],
  seoDescription:
    "The Imperial Citadel of Thang Long in Hanoi is a UNESCO World Heritage Site and the former political centre of Vietnam for over 1,300 years - a layered archaeological complex spanning dynasties from the 7th century to the 20th century, with underground excavations, restored palace foundations, and a unique night tour experience.",
  tips: [
    "The full complex covers two separate areas: the main citadel site at 19C Hoàng Diệu and the Archaeological Site at 18 Hoàng Diệu - both are included in the entry ticket and the archaeological site is a short walk from the main entrance",
    "The Northern Gate of the Thang Long Citadel (Cửa Bắc) sits at the intersection of Phan Đình Phùng and Đặng Dung streets, approximately 550 metres from the main archaeological site - it requires exiting the compound and walking north. Covered under the same entry ticket.",
    "The Night Tour 'Decoding Thang Long Imperial Citadel' runs every Friday and Saturday at 7:00 PM - book in advance at +84 848455222. The 1.5-hour experience includes art performances on the archaeological glass floor, an incense ceremony at Kinh Thien Palace, laser light artifact decoding, and lotus tea under the bodhi tree",
    "The underground archaeological excavations at 18 Hoàng Diệu are among the most significant in Southeast Asian history - the layers of foundation stones, wells, drains, and ceramics span over 1,300 years of continuous occupation",
    "Allow at least 2-3 hours for a thorough visit covering both the main site and the archaeological building - the artifact galleries alone warrant 45-60 minutes",
    "Keep your entry ticket with you throughout the visit - when moving between the main site, the 18 Hoang Dieu archaeological building, and the Northern Gate area, you may be asked to show it again. All three are covered under the same ticket.",
  ],
  content: {
    intro:
      "The Imperial Citadel of Thang Long (Hoàng Thành Thăng Long) in Hanoi's Ba Đình district is one of the most historically significant sites in Southeast Asia and a UNESCO World Heritage Site since 2010. The complex occupies the former political and administrative heart of Vietnam for over 1,300 years, from the construction of the first citadel by Emperor Lý Thái Tổ in 1010 through successive Lý, Trần, Lê, and Nguyễn dynasties, continuing into the 20th century when the site served as the military headquarters from which General Võ Nguyên Giáp directed the wars against France and the United States. The UNESCO inscription recognises the site's outstanding universal value as evidence of the long interaction of diverse cultures across a continuous period of human settlement. The visible remains include restored palace gates, the foundations of Kinh Thien Palace, the Hanoi Flag Tower dating to 1812, wartime bunkers and command facilities from the 20th century, and the extraordinary underground archaeological excavations at 18 Hoàng Diệu - a site of international archaeological significance where layers of palace foundations, ceramics, drainage systems, and structural remains from successive dynasties have been exposed and are displayed in situ.",
    howToGetThere:
      "The main entrance to the Imperial Citadel is at 19C Hoàng Diệu street in Ba Đình district, approximately 1.5km from Hoan Kiem Lake and adjacent to the Vietnam Military History Museum. From the Old Quarter, Grab or taxi takes 10-15 minutes. On foot from Hoan Kiem Lake, the walk takes approximately 20-25 minutes via Điện Biên Phủ street. The site is also within easy walking distance of the Ho Chi Minh Mausoleum complex - the two sites are commonly combined in a Ba Đình half-day itinerary. The Northern Gate (Cửa Bắc) is a separate structure on Phan Đình Phùng street, approximately 550 metres from the main archaeological site on foot.",
    whatToExpect:
      "The main site at 19C Hoàng Diệu covers the restored citadel gates, the Kinh Thien Palace foundations with their elaborate stone dragon staircases, the Hanoi Flag Tower, the Hậu Lâu (Princess Pagoda), wartime bunkers including the D67 command house used during the American War, and the artifact exhibition buildings. The archaeological site at 18 Hoàng Diệu - accessible through the same ticket - displays the excavated foundations of successive palace complexes through a series of glass-floored walkways and open excavation pits, allowing visitors to look directly down onto the exposed archaeological layers. The full visit covering both areas takes 2-3 hours at a comfortable pace. The Night Tour on Friday and Saturday evenings transforms the experience significantly: the site is lit atmospherically, performers in period costume recreate court scenes, and a laser light show is used to interpret the archaeological finds.",
    travelTips:
      "The Imperial Citadel is one of the most important historical sites in Vietnam and warrants more time than most visitors allocate - the combination of restored imperial architecture, 20th century military history, and world-class archaeological excavations in one compact site is genuinely unusual. The Night Tour is particularly recommended for visitors with a specific interest in history or those who have already seen the site by day - the theatrical presentation is well-produced and the after-hours atmosphere of the citadel grounds is markedly different from the daytime visit. Book the night tour in advance by phone as capacity is limited: +84 848455222.",
  },
  insights: {
    highlights: [
      "Walk through 1,300 years of Vietnamese imperial history in a single UNESCO World Heritage site - from Ly dynasty foundations to 20th century war bunkers",
      "View the in-situ archaeological excavations at 18 Hoang Dieu - among the most significant archaeological discoveries in Southeast Asia",
      "Experience the Night Tour on Friday or Saturday evenings: court performances, laser light shows, incense ceremony at Kinh Thien Palace, and lotus tea under the bodhi tree",
    ],
    thingsToKnow: {
      crowds: "Weekday mornings are quietest; public holidays and weekends attract large domestic tour groups - arrive early",
      difficulty: "Flat walking site, fully accessible; the underground archaeological section involves ramps and glass walkways",
      safety: null,
      accessibility: "Main site is accessible with ramps throughout; some areas of the archaeological excavation require steps",
      seasonal: "Pleasant year-round; avoid visiting during major Vietnamese public holidays when the site is at maximum capacity",
    },
    visitorTips: [
      "Buy tickets at the entrance gate - online booking is also available at vedientu.hoangthanhthanglong.com",
      "The audio guide adds significant context to the archaeological sections - worth renting at the entrance",
      "Combine with the Vietnam Military History Museum directly across the road for a full Ba Đình historical half-day",
    ],
    faq: [
      {
        question: "What is the difference between the main site and the 18 Hoang Dieu archaeological site?",
        answer: "Both are included in the same entry ticket. The main site at 19C Hoang Dieu covers the above-ground citadel structures - gates, palace foundations, flag tower, and wartime buildings. The 18 Hoang Dieu site displays the underground archaeological excavations in specially designed viewing buildings. Both are connected within the complex.",
      },
      {
        question: "How do I book the Night Tour?",
        answer: "Contact the Night Tour booking line directly: +84 848455222. The tour runs every Friday and Saturday at 7:00 PM and lasts approximately 1.5 hours. Price is 300,000 VND for adults, 150,000 VND for children under 12, free for children under 5. Arrive at 19C Hoang Dieu at least 15 minutes before departure.",
      },
      {
        question: "Where is the Northern Gate and how do I get there?",
        answer: "The Northern Gate (Cửa Bắc) is a separate structure on Phan Dinh Phung street, approximately 550 metres from the main archaeological site. It requires exiting the main compound and walking north - it is not connected to the ticketed area and can be viewed freely from the street.",
      },
    ],
    sentiment: {
      positive: "Visitors consistently rate the archaeological site as the highlight - the scale and depth of the excavations are unexpected and impressive. The Night Tour receives strong reviews for its atmosphere and production quality.",
      negative: "Some visitors find the above-ground restored structures underwhelming compared to the archaeological sections; English signage in the artifact galleries can be sparse in places.",
    },
  },
}
