import { Location } from "@/data/location"
import { heroUrl } from "@/lib/cloudinary"

export const triAnLake: Location = {
  slug: "tri-an-lake",
  name: "Tri An Lake",
  updatedAt: "2026-09-05",
  provinces: ["dong-nai"],
  destination: "",
  lat: 11.11483833366682,
  lng: 107.03177682081524,
  address: "Hồ Trị An, Vĩnh Cửu, Đồng Nai",
  type: ["lake", "nature"],
  categories: ["nature"],
  experiences: ["camping", "kayaking", "boat-tour", "cycling", "photography", "wildlife", "swimming"],
  tags: ["⛺ Camping", "🌅 Sunset Views", "🏕️ Weekend Escape"],
  bestTime: "Roughly Sep/Oct - Jun is the best overall window, split into two distinct phases: Sep-Dec, when the lake is at its fullest and most scenic for boating and photos, and Jan-Jun, when receding water exposes wide grassy flats that make excellent, spacious camping ground with vivid sunsets - many consider this the prime camping season. Jul-Sep brings more rain and slicker access roads, though it also brings a striking algae bloom across parts of the lake if you don't mind the weather risk. Within a day, paddleboarding is best done early morning or after 3 PM (the water gets uncomfortably warm at midday), and sunset is typically strongest around 4:45-6:10 PM - check the forecast before heading out, since rain can move in quickly",
  bestMonths: [1, 2, 3, 4, 5, 6, 9, 10, 11, 12],
  entranceFee: "Free (boat hire and camping fees vary by operator: camping from 100,000 VND/person)",
  openingHours: "Open all day; camping operators typically run overnight packages",
  mapUrl: "https://www.google.com/maps?q=11.11483833366682,107.03177682081524",
  streetView: {embedUrl: "https://www.google.com/maps/embed?pb=!4v1775890471853!6m8!1m7!1sCAoSF0NJSE0wb2dLRUlDQWdJRHl3N2ppc1FF!2m2!1d11.10127826261844!2d107.0482808137099!3f0.5058173368232985!4f-13.942670503370877!5f0.7820865974627469", },
  heroImage: heroUrl("tri-an-lake-dong-nai-9_qyk3uu"),
  gallery: [
    "tri-an-lake-dong-nai-4_y74889",
    "tri-an-lake-dong-nai-2_vtwl1r",
    "tri-an-lake-dong-nai-5_zderza ",
    "tri-an-lake-dong-nai_lv646r",
    "tri-an-lake-dong-nai-3_vmpfgg",
    "tri-an-lake-dong-nai-10_gswdvj",
    "tri-an-lake-dong-nai-7_cuvwsb",
    "tri-an-lake-dong-nai-9_qyk3uu",
    "tri-an-lake-dong-nai-6_pfqup5",
  ],
  seoDescription:
    "Tri An Lake is a large reservoir in Dong Nai province, 65-70km from Ho Chi Minh City, formed by the Tri An hydroelectric dam on the Dong Nai River. A popular weekend camping and kayaking destination for city residents, with 76 forested islands, calm water, and dramatic sunset views.",
  tips: [
    "Book camping spots in advance for weekends between November and April - popular operators on the lake's forested islands fill up fast, particularly around public holidays",
    "Sunrise on the lake is often better than sunset - the eastern orientation of many camping spots and the mist sitting over the water in the early morning make for excellent photography",
    "Bring insect repellent - the forested camping areas have mosquitoes, particularly in the hour after sunset",
    "Kayaking or stand-up paddleboarding around the lake's small islands is the best way to explore - the water is calm and, at sunset, the sky and water colours often blend together strikingly",
    "The lake is also a fishing destination - if you're camping overnight, early morning fishing from the bank or a small boat is worth trying for bass and snakehead",
    "The lake's peaceful, natural shoreline has also made it a popular backdrop for wedding photography - don't be surprised to see a shoot in progress at some of the more scenic spots",
    "Check the cleanliness of your chosen camping spot before setting up - like many popular weekend spots near a big city, trash left by previous visitors can be an issue at some sites",
    "Kayak and SUP rental is typically available at campsites for around 100,000-200,000 VND/hour - when paddling, stick to shallow, grassy-flat areas rather than zones with submerged tree stumps (which can damage a paddleboard's fin), wear a life jacket even if you're a confident swimmer, and don't go more than about 300m from shore if you're unfamiliar with the lake bed",
    "If swimming, avoid deep-water areas entirely and always wear a life jacket - this isn't a supervised beach",
    "The lake supports over 40 fish species and hundreds of bird species, giving it real ecological variety beyond just the scenery",
    "Worth knowing: the lake's shoreline has faced real environmental governance pressure over the years - in 2018, an illegal 2,400-sow pig farm was built right beside the lake in Định Quán district before being forcibly demolished by local authorities, who stated at the time that it had never gone into operation and had not affected the lake's water. It's a reminder that unauthorised development near the shoreline has been a recurring local issue, even if this specific case was caught and resolved before causing harm",
    "For a day trip, a natural loop is: the dam itself, a nearby camping/viewpoint area, then coffee or lunch somewhere with proper seating if you want to linger. For an overnight stay, look at glamping-style setups with everything pre-arranged (tents, bedding, sometimes food) rather than fully independent camping if you'd rather not haul your own gear",
    "If driving from Ho Chi Minh City for the day, leaving around 3 PM gets you there in time for evening without arriving in the harshest midday heat",
  ],
  content: {
    intro:
      "Tri An Lake was formed in 1987 when the Tri An hydroelectric dam was completed on the Dong Nai River, with construction backed by Soviet technical assistance starting in 1984. The lake stretches across four districts of Dong Nai province - Vinh Cuu, Dinh Quan, Thong Nhat, and Trang Bom - flooding what was once forest and farmland. The reservoir covers approximately 323 square kilometres at full capacity and contains 76 islands of varying sizes, making it one of the largest artificial lakes in Vietnam; the dam itself is well-known enough to appear on Vietnam's 5,000 VND banknote. The lake is effectively split into a larger main body and a smaller secondary lake, with most camping operators concentrated around the smaller lake, which also has a reputation for excellent sunrise views. A small town has grown up around parts of the shoreline, with grocery stores and food stalls covering basic needs. Its forested shoreline and scattered islands have made it a popular escape from Ho Chi Minh City for residents who want camping, water activities, and natural scenery within a couple of hours' drive. The lake borders Vinh Cuu Nature Reserve on its northern shore, and sections of the forest around the lake retain good canopy cover with populations of birds and freshwater fish. The dam itself remains an active power station supplying electricity to the southern grid.",
    howToGetThere:
      "Tri An Lake is roughly 65-70km from central Ho Chi Minh City, approximately 1.5-2 hours by motorbike or car via Highway 1 through Bien Hoa and then north into Dong Nai province, though exact travel time depends heavily on Bien Hoa traffic. Most visitors with camping or boat tour bookings receive specific pickup point directions from their operators, as the lake's access roads vary by section - some remote camping spots involve a final stretch of tree-lined road through forest to reach the water. The main public access area is near the dam site in Thanh Phu commune. From Bien Hoa city, the lake's closest points are about 25-30km - reachable by xe ôm.",
    whatToExpect:
      "The lake experience varies significantly depending on which section you access and whether you're camping or doing a day visit. The island camping experience - typically on one of the lake's small forested islands accessible by boat - is the most popular format: operators provide tents, hammocks, basic cooking equipment, and boat transfers. The water is clear enough for kayaking, paddleboarding, and swimming in the dry season. Day-trip boat tours typically circle the larger islands and stop at fishing spots. Đảo Chim Ó and Đảo Đồng Trường are among the best-established tourist islands, reached by speedboat from the Đồng Trường ferry pier (about 45 minutes each way) and worth booking food in advance for if visiting on a weekend, while several other islands and lakeside sites - including Đảo Cao Minh (camping, bungalows, SUP, and food service) and Đảo Sói (camping and SUP) - now also offer their own camping and outdoor activities. The far end of Đảo Chim Ó has a scenic stone-paved lakeside path, best for photos early in the morning before it gets busy. Cyclists can also ride out through Mã Đà Forest, part of the Đồng Nai Culture-Nature Reserve and tied to the historic 'Chiến khu Đ' wartime resistance base - a popular route runs roughly 10-15km depending on fitness. The Tri An dam itself, built in the 1980s with Soviet assistance, is a sightseeing stop in its own right, with the dam body and spillway tower best viewed from below the dam or from the bridge crossing the Dong Nai River. Sunsets over the lake in the dry season can be very striking - the combination of flat water, silhouetted forest, and open western sky creates the kind of light that photographs well without much effort, and the scene is popular with wedding photographers as well as casual visitors.",
    travelTips:
      "Tri An functions primarily as a weekend destination for Ho Chi Minh City residents rather than an international tourism site, which means the infrastructure is geared toward group bookings and overnight packages rather than individual drop-in visits. The most straightforward approach for solo travellers or couples is to book through one of the established camping operators who run regular weekend packages - these typically include boat transfer, tent, meals, and kayak use for around 400,000 - 600,000 VND per person. Weekday evenings are noticeably calmer than weekends, when the lake draws real crowds and noise from other campers. The lake is genuinely pleasant rather than spectacular, but the combination of relatively easy access from the city, calm water, and forested setting makes it one of the better quick-escape options in the south for travellers based in Ho Chi Minh City for more than a few days.",
  },
  insights: {
    highlights: [
      "Stand-up paddleboarding at sunset, when the water often mirrors the colours of the sky",
      "76 islands scattered across the lake, with some remote camping spots reached via a scenic drive through tree-lined forest roads",
      "A popular backdrop for wedding photography thanks to its peaceful, natural lakeside scenery",
    ],
    thingsToKnow: {
      crowds: "Weekends attract real crowds and noise; evenings and weekdays are far more serene and peaceful",
      difficulty: null,
      safety: null,
      accessibility: "Some of the more remote camping spots require driving a fair distance on forest roads to reach - make sure you have reliable navigation and enough fuel before heading in",
      seasonal: null,
    },
    visitorTips: [
      "Visit during weekday evenings for tranquility; weekends are crowded and noisy",
      "Allow around 1.5-2 hours of driving time from HCMC, depending on traffic through Bien Hoa",
      "Check trash and cleanliness conditions at your chosen camping spot before setting up",
    ],
    faq: [
      { question: "How crowded is Tri An Lake on weekends?", answer: "Daytime weekends are very crowded and noisy, especially around public holidays. Evenings and weekdays are much more peaceful and serene for visitors." },
      { question: "How far is Tri An Lake from Saigon?", answer: "Roughly 65-70km from central Ho Chi Minh City, taking about 1.5-2 hours by motorbike or car depending on traffic through Bien Hoa." },
      { question: "What activities are available at Tri An Lake?", answer: "Camping, kayaking, stand-up paddleboarding, fishing, boat tours around the islands, and photography - the lake is also a popular spot for wedding shoots." },
    ],
    sentiment: {
      positive: "Visitors praise the natural scenery, tranquility, sunset views, and proximity to Saigon as ideal for weekend escapes and water activities.",
      negative: "Some visitors report trash and cleanliness issues at certain camping sites, particularly after busy weekends, and the lake's shoreline has occasionally faced illegal development pressure (including a demolished illegal pig farm in 2018) that speaks to broader environmental oversight challenges in the area.",
    },
  },
}
